...
export default (expr, index=0, len=expr.length) => {
  ...
  gobbleExpression = () => {
    ...
    while ((biop = gobbleBinaryOp())) {
      if (!(prec = binary[biop])) { index -= biop.length; break; }
      ...
      const comparePrev = prev => prec <= prev[1];
      ...
      if (!node) err("Expected expression after " + cur_biop);
