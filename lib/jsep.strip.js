const MAX_ULEN = 3, MAX_BLEN = 3,
  ...
  binaryPrecedence = (op_val) => binary_ops[op_val] || 0,
  isIdentifierStart = (ch) => (ch >= 65 && ch <= 90) || // A...Z
      (ch >= 97 && ch <= 122) || // a...z
      (ch >= 128 && !binary_ops[String.fromCharCode(ch)]) || // any non-ASCII that is not an operator
      ID_CHAR.has(String.fromCharCode(ch)) // additional characters
  ,
...
class Jsep {
  ...
  gobbleBinaryExpression() {
    ...
    biop_info = { value: biop, prec: binaryPrecedence(biop), right_a: right_associative.has(biop) };
    ...
    while ((biop = this.gobbleBinaryOp())) {
      ...
      biop_info = { value: biop, prec, right_a: right_associative.has(biop) };
