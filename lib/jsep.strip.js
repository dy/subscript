...
export default (expr, index=0, len=expr.length) => {
  ...
  gobbleToken = () => {
    ...
    if (!isNotQuote(cc)) index++, node = new String(gobble(isNotQuote)), index++ // string literal
...
// FIXME: this must be part of precedence sort
  gobbleTokenProperty = (node) => {
    ...
    let ch = code();
    while (ch === PERIOD || ch === OBRACK || ch === OPAREN) {
      index++;
      if (ch === OBRACK) node = ['[', node].concat(gobbleSequence(CBRACK)||[]) // MEMBER_EXP
        ...
        skip(isSpace);
        node = ['.', node, gobble(isIdentifierPart)] // MEMBER_EXP
      }
