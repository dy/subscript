...
test.fork('perf: justin <= jsep*1.15', {data: {RUNS, src}}, async ({ ok }, {RUNS, src}) => {
  const bench = (fn) => {
    let best = Infinity
    for (let r = 0; r < 3; r++) {
      let t = performance.now()
  ...
  ok(time <= baseline * 1.15, `justin (${time.toFixed(0)}ms) should be <= jsep (${baseline.toFixed(0)}ms) * 1.15`)
})
