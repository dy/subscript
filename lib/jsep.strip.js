...
export default (expr, index=0) => {
  ...
  gobbleToken = () => {
    ...
    if (isDecimalDigit(cc) || cc === PERIOD_CODE) return gobbleNumericLiteral();

    // Single or double quotes
    if (cc === SQUOTE_CODE || cc === DQUOTE_CODE) node=new String(expr.slice(index++, skipTo(c)))
    else if (cc === OBRACK_CODE) node = gobbleArray();
    else {
      ...
      if (isIdentifierStart(cc)) {
        ...
        if (node.name in literals) node = node.name // LITERAL, TODO: map literal after
      }
      // open parenthesis
      else if (cc === OPAREN_CODE) node = gobbleGroup();
    }
  ...
  // skip index to the character occurence
  skipTo = (s,n) => (n=expr.indexOf(s,index))<0 ? err('Unclosed ' + s) : index=n+s.length,
  ...
  gobbleNumericLiteral = () => {
    ...
    if (ch === 'e' || ch === 'E') { // exponent marker
      ...
      while (isDecimalDigit(code())) number += expr.charAt(index++);// exponent itself
