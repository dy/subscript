...
export default (expr, index=0, len=expr.length) => {
  ...
  gobbleExpressions = (untilICode) => {
    ...
    while (index < len) {
      ...
      if (ch_i === SEMCOL_CODE || ch_i === COMMA_CODE) index++; // ignore separators
      else {
  ...
  gobbleToken = () => {
    ...
    if (!isNotQuote(cc)) node=new String(expr.slice(++index, skip(isNotQuote))), index++
    else if (cc === OBRACK_CODE) index++, node = [Array, ...gobbleArguments(CBRACK_CODE)] // gobbleArray
    else {
      const MAX_ULEN = 3
  ...
  gobbleGroup = () => {
    ...
    if (code() !== CPAREN_CODE) err('Unclosed (');
    index++;
