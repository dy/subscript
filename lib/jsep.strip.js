const isDigit = c => c >= 48 && c <= 57, // 0...9,
  ...
  cmd = a => Array.isArray(a) && a.length && a[0] && (typeof a[0] === 'string' || cmd(a[0])) , // is calltree node
...
parse = (expr, index=0, len=expr.length) => {
  ...
  gobbleToken = () => {
    ...
    if (isDigit(cc) || cc === PERIOD) lit(node = new Number(gobbleNumber()), +node);
    ...
    while (skip(isSpace), cc=code(), cc === PERIOD || cc === OBRACK || cc === OPAREN) {
      ...
      else if (cc === PERIOD) node = ['.', node, lit(new String(gobble(isIdentifierPart)))] // MEMBER_EXP
    }
...
evaluate = (s, ctx={}, n) =>
  ...
  cmd(s) ?
    (c=s[0], cmd(c) ? evaluate(c, ctx) : ctx[c] || (s.length<3?unary:binary).find(op => op[c]) || Err('Unknown comand ' + c))
