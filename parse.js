...
// a + b - c
expr = (arg=0, cc=space(), node, from=idx, i=0, mapped, prec, end) => {
  // prefix or token
  while (from===idx && i < token.length) node = token[i++](cc)

  // prec may have end character in hi orders
  // FIXME: better make a separate argument
  if ((arg&0xff)>SPACE) end=arg, prec=0; else end=(prec&0xff00)>>8, prec=arg

  // postfix or binary
  for (cc=space(), i=prec; i < operator.length;)
    if (cc===end) break
    else if (mapped = operator[i++](node, cc, i|end<<8)) node = mapped, i=prec, cc=space(); // we pass i+1 as precision
...
// (...exp)
group = (c, node) => c === OPAREN ? (idx++, node = expr(CPAREN), idx++, node) : '',
// var or literal
id = name => skip(c =>
  (
    (c >= 48 && c <= 57) || // 0..9
...
operator = [
  ...
  // '|': 8, '^': 9, '&': 10,
  (a,cc,prec) => cc===OR ? [skip(),a,expr(prec)] : null,
  (a,cc,prec) => cc===HAT ? [skip(),a,expr(prec)] : null,
  (a,cc,prec) => cc===AND ? [skip(),a,expr(prec)] : null,
  // '==': 11, '!=': 11,
  (a,cc,prec) => cc===EQ&&cc===code(1) ? [skip(),a,expr(prec)] : null,
  // '<': 12, '>': 12, '<=': 12, '>=': 12,
  (a,cc,prec) => cc===GT||cc===LT ? [skip(),a,expr(prec)] : null,
  // '<<': 13, '>>': 13, '>>>': 13,
  (a,cc,prec) => (cc===LT||cc===GT)&&cc===code(1) ? [skip(cc===code(2)?3:2), a, expr(prec)] : null,
  // '+': 14, '-': 14,
  (a,cc,prec) => (cc===PLUS || cc===MINUS) && a!=='' && code(1) !== cc ? [skip(), a, expr(prec)] : null,
  // '*': 15, '/': 15, '%': 15
  (a,cc,prec) => (cc===MUL && code(1) !== MUL) || cc===DIV || cc===MOD ? [skip(), a, expr(prec)] : null,
  // -- ++ unaries
  (a,cc,prec) => (cc===PLUS || cc===MINUS) && code(1) === cc ? [skip(2), a===''?expr(prec):a] : null,
  // - + ! unaries
  (a,cc,prec) => (cc===PLUS || cc===MINUS || cc===EXCL)&&a==='' ? [skip(1), expr(prec)] : null,
  // '()', '[]', '.': 18
  (a,cc,prec,arg) => (
    // a.b[c](d)
    cc===PERIOD ? [skip(), a , '"'+(space(), id())+'"'] :
    cc===OBRACK ? [(skip(),'.'), a, expr(CBRACK)] :
    cc===OPAREN ? (
