...
const code = () => cur.charCodeAt(index), // current char code
  ...
  map = (node, t) => isCmd(node) ? (t = transforms[node[0]], t?t(node):node) : node


export const parse = expr => (cur=expr, index=0, group(lastOp = ['',0])),
...
group = (curOp, rootEnd=end, curEnd) => {
  ...
  if (curEnd = groups[curOp[0]]) end = curEnd // also we write root end marker
  ...
  tokens.find(token => (node = token()) !== '')

  space()

  // consume expression for current precedence or group (== highest precedence)
  while ((op = operator(binary)) && (curEnd || op[1] > curOp[1])) {
    node = [op[0], node]
...
unary = op => (op = operator(prefix)) && map([op[0], group(op)]),
...
groups = {'(':')','[':']'}, // FIXME: consolidate under group
tokens = [float, string, id, unary],
