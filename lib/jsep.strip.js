...
export default (expr, index=0, len=expr.length) => {
  ...
  gobbleToken = () => {
    ...
    else if (cc === OBRACK) index++, node = [Array, ...gobbleSequence(CBRACK)] // array
    else {
      const MAX_ULEN = 3
...
// FIXME: this must be part of precedence sort
  gobbleTokenProperty = (node) => {
    ...
    while (ch === PERIOD || ch === OBRACK || ch === OPAREN || ch === QUMARK) {
      ...
      else if (ch === OPAREN) node = [node, ...gobbleSequence(CPAREN)] // CALL_EXP
  ...
  gobbleGroup = () => {
    index++;
    let nodes = gobbleSequence(CPAREN);
    return nodes.length < 2 ? nodes[0] : nodes
  }

  // do parse
  const nodes = gobbleSequence();
